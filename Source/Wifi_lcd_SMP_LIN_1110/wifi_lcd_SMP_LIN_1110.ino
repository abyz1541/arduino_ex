...
char lcdLine1[17]="Smart IoT By PKJ";
...
void loop()
{
  ...
  // 인터럽트로 토글기능구현 ( 회로에 콘덴서 추가하여 지연회로로 체터링은 제거)
  ...
  if(ext0Flag)
  {
    if(digitalRead(LED_BUILTIN_PIN))
    {
      ...
      sprintf(sendBuf,"[LHO_SMP]BUTTON@ON\r\n");
      client.write(sendBuf,strlen(sendBuf));
      client.flush();
    }else{
      digitalWrite(LED_BUILTIN_PIN, HIGH);
      
      sprintf(sendBuf,"[LHO_SMP]BUTTON@OFF\r\n");
      client.write(sendBuf,strlen(sendBuf));
      client.flush();
    }
    ext0Flag = false;
...
void socketEvent()
{
  ...
  else if(!strncmp(pArray[1],"LAMP",4)) //LAMP@ON/OFF 명령어 처리 (D3 LED On/off)
  {
